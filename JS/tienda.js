...
function cargarProductos() {
    ...
    const busqueda = encodeURIComponent(buscador.value.trim());
    const categoria = encodeURIComponent(filtroCategoria.value);

    fetch(`../PHP/mostrarpublicaciones.php?busqueda=${busqueda}&categoria=${categoria}`)
    .then(res => res.json())
    .then(datos => {
        ...
        if (datos.length === 0) {
            productos.innerHTML = "<p>No se encontraron publicaciones</p>";
            return;
        }
...
buscador.addEventListener("input", () => {
    ...
    temporizadorBusqueda = setTimeout(cargarProductos, 400);
});
